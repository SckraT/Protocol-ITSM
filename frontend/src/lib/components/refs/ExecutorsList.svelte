<script lang="ts">
  ...
  async function add() {
    ...
    const ok = await refsStore.createExecutor(newName.trim(), newDept ? Number(newDept) : null);
    if (ok) {
      newName = '';
      newDept = '';
    }
  ...
  function startEdit(id: number, name: string, deptId: number | null) {
    editingId = id;
    editName = name;
            ...
            <li class="flex items-center gap-2 rounded px-2 py-1.5 hover:bg-[var(--table-hover)]">
              ...
              {:else}
                ...
                <button
                  ...
                  onclick={() => startEdit(exec.id, exec.name, exec.department_id)}
                >
                  <Pencil size={14} />
                </button>
                <button
